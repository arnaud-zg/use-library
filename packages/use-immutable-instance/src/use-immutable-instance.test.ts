...
// First: Test the TodoList class in isolation
describe("TodoList class", () => {
  it("should initialize with given items", () => {
    const todoList = new TodoList(["Item 1"]);
    expect(todoList.items).toEqual(["Item 1"]);
    expect(todoList.getCount()).toBe(1);
  });

  it("should add items immutably", () => {
    const todoList = new TodoList([]);
    const newList = todoList.addItem("Item 1");

    expect(todoList.items).toEqual([]);
    expect(newList.items).toEqual(["Item 1"]);
    expect(newList.getCount()).toBe(1);
  });

  it("should remove items immutably", () => {
    const todoList = new TodoList(["Item 1", "Item 2"]);
    const newList = todoList.removeItem(0);

    expect(todoList.items).toEqual(["Item 1", "Item 2"]);
    expect(newList.items).toEqual(["Item 2"]);
    expect(newList.getCount()).toBe(1);
  });
  ...
  it("should support method chaining", () => {
    ...
    expect(newList.items).toEqual(["Item 1", "Item 2", "Item 3"]);
    expect(newList.getCount()).toBe(3);
  });
});

// Second: Test integration with useImmutableInstance hook
describe("useImmutableInstance hook", () => {
